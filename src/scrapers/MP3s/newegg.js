...
const settings = {
  ...
  datastoreKind: 'MP3s'  // The kind for the new entity
};
...
function scrapeStartPage(url) {
  return new Promise((resolve, reject) => {
    ...
    .set({
      ...
      'additionalFeatures': 'fieldset> dl > dt:contains("Additional Features") + dd:html',
      'wireless': 'fieldset> dl > dt:contains("Wireless") + dd',
      'connectionType': 'fieldset> dl > dt:contains("Connection Type") + dd',
      'powerAdapter': 'fieldset> dl > dt:contains("Power Adapter") + dd'
    })
    ...
    .then(function(context, data, next) {
      ...
      if (typeof data.additionalFeatures !== 'undefined') {
        data.additionalFeatures = data.additionalFeatures.split(/<br>|,/g);
      }
...
// MP3 Player Pages
for (var i = 0; i < 12; i++) {
  let urlString = "www.newegg.com/Product/ProductList.aspx?Submit=ENE&N=100007878%204814&IsNodeId=1&bop=And&Page=" + i + "&PageSize=96";
