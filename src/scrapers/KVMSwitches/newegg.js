...
const settings = {
  ...
  datastoreKind: 'KVMSwitches'  // The kind for the new entity
};
...
function scrapeStartPage(url) {
  return new Promise((resolve, reject) => {
    ...
    .set({
      ...
      'pcConnectors': 'fieldset> dl > dt:contains("PC Connectors") + dd:html',
      'consoleConnectors': 'fieldset> dl > dt:contains("Console Connectors") + dd:html',
      'computerConnections': 'fieldset> dl > dt:contains("Computer Connections") + dd',
      'monitorConnections': 'fieldset> dl > dt:contains("Monitor Connections") + dd',
      'usbFeatures': 'fieldset> dl > dt:contains("USB Features") + dd',
      'monitorsSupported': 'fieldset> dl > dt:contains("Monitors Supported") + dd',
      'powerFeatures': 'fieldset> dl > dt:contains("  Power Features") + dd',
      'outletType': 'fieldset> dl > dt:contains("Electrical Outlet Plug Type") + dd',
      'videoResolution': 'fieldset> dl > dt:contains("Video Resolution") + dd'
    })
    ...
    .then(function(context, data, next) {
      ...
      if (typeof data.type !== 'undefined') {
        data.type = data.type.split(/<br>|,/g);
      }
      if (typeof data.pcConnectors !== 'undefined') {
        data.pcConnectors = data.pcConnectors.split(/<br>|,/g);
      }
      if (typeof data.consoleConnectors !== 'undefined') {
        data.consoleConnectors = data.consoleConnectors.split(/<br>|,/g);
      }
...
// KVM Switches
for (var i = 0; i < 26; i++) {
  let urlString = "www.newegg.com/Product/ProductList.aspx?Submit=ENE&N=100160866%204814&IsNodeId=1&bop=And&Page=" + i + "&PageSize=96";
