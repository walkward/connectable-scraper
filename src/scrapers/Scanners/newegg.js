...
const settings = {
  ...
  datastoreKind: 'Scanners'      // The kind for the new entity
};
...
function scrapeStartPage(url) {
  return new Promise((resolve, reject) => {
    ...
    .set({
      ...
      'usbPorts': 'fieldset> dl > dt:contains("USB Ports") + dd',
      'operatingSystemsSupported': 'fieldset> dl > dt:contains("Operating Systems Supported") + dd:html',
      'otherPorts': 'fieldset> dl > dt:contains("Other Ports") + dd',
      'interface': 'fieldset> dl > dt:contains("Interface") + dd'
    })
    ...
    .then(function(context, data, next) {
      ...
      if (typeof data.operatingSystemsSupported !== 'undefined') {
        data.operatingSystemsSupported = data.operatingSystemsSupported.split(/<br>|,/g);
      }
...
// Document Scanners
for (var i = 0; i < 8; i++) {
  let urlString = "www.newegg.com/Product/ProductList.aspx?Submit=ENE&N=100093096%204814&IsNodeId=1&bop=And&Page=" + i + "&PageSize=96";
  promiseStack.push(scrapeStartPage(urlString));
}

// Specialized Scanners
for (var i = 0; i < 4; i++) {
  let urlString = "www.newegg.com/Product/ProductList.aspx?Submit=ENE&N=100093097%204814&IsNodeId=1&bop=And&Page=" + i + "&PageSize=96";
  promiseStack.push(scrapeStartPage(urlString));
}

// Flatbed Scanners
for (var i = 0; i < 2; i++) {
  let urlString = "www.newegg.com/Product/ProductList.aspx?Submit=ENE&N=100093098%204814&IsNodeId=1&page=" + i + "&bop=And&PageSize=96";
  promiseStack.push(scrapeStartPage(urlString));
}
