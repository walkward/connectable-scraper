...
const settings = {
  ...
  datastoreKind: 'Camcorders' // The kind for the new entity
};
...
for (var i = 0; i < 25; i++) {
  let urlString = "www.newegg.com/Product/ProductList.aspx?Submit=ENE&N=100023509%204814&IsNodeId=1&bop=And&Page=" + i + "&PageSize=96";
  promiseStack.push(scrapeStartPage(urlString));
}

// Consumer Camcorders
for (var i = 0; i < 3; i++) {
  let urlString = "www.newegg.com/Consumer-Camcorders/SubCategory/ID-3542/Page-" + i + "?Tid=56757&PageSize=96";
  promiseStack.push(scrapeStartPage(urlString));
}
